...
export default function Recruitment() {
  ...
  const createJob = useMutation({ mutationFn: (body: typeof newJob) => fetch(`${API}/recruitment/jobs`, { method: "POST", headers: apiHeaders(token), body: JSON.stringify(body) }).then(r => r.json()), onSuccess: () => { toast.success("Job posting created successfully"); qc.invalidateQueries({ queryKey: ["jobs"] }); setShowJobForm(false); setNewJob({ title: "", location: "", type: "full_time", salaryMin: "", salaryMax: "", description: "", status: "open" }); }, onError: () => toast.error("Failed to create job posting", { description: "Please check your input and try again." }) });
  const createApp = useMutation({ mutationFn: (body: object) => fetch(`${API}/recruitment/applications`, { method: "POST", headers: apiHeaders(token), body: JSON.stringify(body) }).then(r => r.json()), onSuccess: () => { toast.success("Application created successfully"); qc.invalidateQueries({ queryKey: ["applications"] }); setShowAppForm(false); }, onError: () => toast.error("Failed to create application", { description: "Please try again." }) });
  const moveApp = useMutation({ mutationFn: ({ id, stage }: { id: number; stage: string }) => fetch(`${API}/recruitment/applications/${id}`, { method: "PATCH", headers: apiHeaders(token), body: JSON.stringify({ stage }) }).then(r => r.json()), onSuccess: () => { toast.success("Application moved successfully"); qc.invalidateQueries({ queryKey: ["applications"] }); }, onError: () => toast.error("Failed to move application", { description: "Please try again." }) });
  const closeJob = useMutation({ mutationFn: (id: number) => fetch(`${API}/recruitment/jobs/${id}`, { method: "PATCH", headers: apiHeaders(token), body: JSON.stringify({ status: "closed" }) }).then(r => r.json()), onSuccess: () => { toast.success("Job posting closed successfully"); qc.invalidateQueries({ queryKey: ["jobs"] }); }, onError: () => toast.error("Failed to close job posting", { description: "Please try again." }) });
