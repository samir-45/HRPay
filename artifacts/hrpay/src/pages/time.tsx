...
export default function Time() {
  ...
  const createMut = useCreateTimeEntry({ mutation: { onSuccess: () => { qc.invalidateQueries({ queryKey: getListTimeEntriesQueryKey({}) }); setShowModal(false); setForm({ employeeId: "", date: new Date().toISOString().split("T")[0], clockIn: "", clockOut: "", hoursWorked: "", notes: "" }); } } });
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{entryList.length} entries · {totalHours.toFixed(1)} total hrs · {totalOT.toFixed(1)} OT hrs</p>
        </div>
