...
router.post("/auth/login", async (req, res) => {
  ...
  res.json({ token, user: { id: user.id, email: user.email, name: user.name, role: user.role, companyId: user.companyId ?? null } });
});
...
router.get("/auth/me", (req, res) => {
  ...
  try {
    ...
    res.json({ id: payload.id, email: payload.email, name: payload.name, role: payload.role, companyId: payload.companyId ?? null });
  } catch {
